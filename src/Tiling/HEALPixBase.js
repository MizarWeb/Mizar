...
import HealPixTables from "./HEALPixTables";
import Long from "../Utils/Long";
import CircleFinder from "../Utils/CircleFinder";
...
var loc2pix = function (order, phi, theta) {
  ...
  if (za <= 2.0 / 3.0) {
    ...
    if (z >= 0) {
      return xyf2nest(
        lNside.subtract(jm).subtract(lOne).toInt(),
        lNside.subtract(jp).subtract(lOne).toInt(),
        ntt,
        order
      );
    } else {
