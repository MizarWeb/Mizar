...
import BoundingBox from "../Renderer/BoundingBox";
...
Tile.prototype.computePosition = function (u, v) {
  ...
  for (var i = 0; i < 3; i++) {
    vec[i] =
      (1.0 - vFrac) * (1.0 - uFrac) * this.vertices[vertexOffset + i] +
...
Tile.prototype.needsToBeRefined = function (renderContext) {
  ...
  var pixelSize =
    radius / (px * pixelSizeVector[0] + py * pixelSizeVector[1] + pz * pixelSizeVector[2] + pixelSizeVector[3]);
