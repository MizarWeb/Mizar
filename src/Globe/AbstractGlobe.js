...
define([
    ...
    function _checkOptions(options) {
        if (!options.coordinateSystem) {
            throw new ReferenceError(
                "coordinateSystem is not defined in " + JSON.stringify(options),
                "AbstractGLobe.js"
            );
        }
    }
    ...
    AbstractGlobe.prototype._updateGeoTiling = function(oldCrs, crs) {
        ...
        if (
            crs.isProjected() &&
            crs.getProjection().getName() === Constants.PROJECTION.Azimuth
        ) {
            if (
                oldCrs.isProjected() &&
                oldCrs.getProjection().getName() ===
                    Constants.PROJECTION.Azimuth
            ) {
                        ...
                        while (geometryIdx--) {
                            ...
                            var tileIndices =
                                vector.maxTilePerGeometry > 0
                                    ? this.tileManager.tiling.getOverlappedLevelZeroTiles(
                                          geometry
                                      )
                                    : null;
                            // update
