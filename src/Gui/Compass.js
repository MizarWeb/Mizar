...
define(["jquery", "../Utils/Constants","../Services/CompassCore", "../Utils/Utils"],
    ...
    var Compass = function (options) {
        console.log("Compass.construct()",options);
        /**
         *    Private variables
        */
        this.svgDoc = null;
    ...
    Compass.prototype.init = function () {
        ...
        document.getElementById(this.parentElement).innerHTML = '<div id="objectCompass"></div>';
        ...
        Compass.prototype.prepare = function () {
            /* Svg interactive elements */
            this.east = this.svgDoc.getElementById("East"); //get the inner element by id
            this.west = this.svgDoc.getElementById("West"); //get the inner element by id
            this.south = this.svgDoc.getElementById("South"); //get the inner element by id
            this.north = this.svgDoc.getElementById("North"); //get the inner element by id
            this.northText = this.svgDoc.getElementById("NorthText");
            this.outerCircle = this.svgDoc.getElementById("OuterCircle");
    };
    ...
    /**************************************************************************************************************/
        ...
        Compass.prototype.attachTo = function (context) {
            ...
            var _dy = 0;
            var dragging = false;
            var _outerCircleRadius = this.outerCircle.ownerSVGElement.clientWidth / 2;
            ...
            this.svgDoc.addEventListener('mousedown', _handleMouseDown);
            ...
            this.svgDoc.addEventListener('mousemove', _handleMouseMove);
            ...
            this.svgDoc.addEventListener('mouseup', _handleMouseUp);

            this.east.addEventListener("click", function () {
                self.ctx.getNavigation().pan(self.panFactor, 0.0);
                CompassCore.updateNorth();
            });

            this.west.addEventListener("click", function () {
                self.ctx.getNavigation().pan(-self.panFactor, 0.0);
                CompassCore.updateNorth();
            });

            this.north.addEventListener("click", function () {
                self.ctx.getNavigation().pan(0, self.panFactor);
                CompassCore.updateNorth();
            });

            this.south.addEventListener("click", function () {
                self.ctx.getNavigation().pan(0, -self.panFactor);
                CompassCore.updateNorth();
            });

            this.northText.addEventListener("click", CompassCore._alignWithNorth);

            if (this.isMobile) {
                var passiveSupported = Utils.isPassiveSupported();
                this.svgDoc.addEventListener('touchstart', _handleMouseDown, passiveSupported ? { passive: true } : false);
                this.svgDoc.addEventListener('touchup', _handleMouseUp);
                this.svgDoc.addEventListener('touchmove', _handleMouseMove, passiveSupported ? { passive: true } : false);
                this.northText.addEventListener("touchstart", CompassCore._alignWithNorth, passiveSupported ? { passive: true } : false);
            }

            // Update fov when moving
            this.ctx.subscribe(Constants.EVENT_MSG.NAVIGATION_MODIFIED, CompassCore.updateNorth);
        ...
        /**
         * Detachs the tracker from the context.
         * @function detach
         * @memberOf Compass#
         */
