...
function _isEqual(date1, date2) {
  ...
  return (
    (interval2.from <= interval1.from && interval1.from <= interval2.to) ||
...
function _binarySearch(requestedTime, requestedPeriodTime, startTime, nbValues, timeResolution) {
  ...
  while (min <= max) {
    ...
    if (_isEqual(requestedTime, currentDate) || _isIntersect(requestedPeriodTime, currentDate)) return guess;
...
Time.prototype.isInSampling = function (samplingTimeDefinition) {
  ...
  var nbValues = Math.floor(
    stopDateTimeDef.diff(startDateTimeDef, timeResolutionDef.unit) / parseInt(timeResolutionDef.step)
  );
