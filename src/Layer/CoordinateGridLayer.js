    ...
    for (phi = phiStart; phi <= phiStop; phi += this.longitudeSample) {
      ...
      for (i = 0; i < this.tesselation; i++) {
        ...
        if (this.gridCrs.getGeoideName() !== Constants.CRS.Equatorial) {
          ...
          geo = this.getGlobe()
            .getCoordinateSystem()
            .convert(geo, this.gridCrs.getGeoideName(), Constants.CRS.Equatorial);
...
CoordinateGridLayer.prototype.computeGeoCenter = function () {
  ...
  var center3d = ray.computePoint(
    ray.sphereIntersect([0, 0, 0], this.getGlobe().getCoordinateSystem().getGeoide().getRadius())
  );
  var geoCenter = [];
  ...
  geoCenter = this.getGlobe()
    .getCoordinateSystem()
    .convert(geoCenter, Constants.CRS.Equatorial, this.gridCrs.getGeoideName());
...
CoordinateGridLayer.prototype.updateLabel = function (label, posGeo) {
  posGeo = this.getGlobe()
    .getCoordinateSystem()
    .convert(posGeo, this.gridCrs.getGeoideName(), Constants.CRS.Equatorial);
...
export default CoordinateGridLayer;
