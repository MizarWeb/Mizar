    ...
    if (_navigation.inertia) {
      ...
      if (geo) {
        ...
        var widthHeightFactor = Math.round(
          _navigation.ctx.getRenderContext().getCanvas().width / _navigation.ctx.getRenderContext().getCanvas().height
        );
    ...
    // Zoom on Right click
      ...
      if (_slower % 3 === 0 && _slower > 1) {
        ...
        if (_dy > 0 && _dy > _dx) {
          ...
          if (_pressedGeo) {
            pos = [
              _navigation.ctx.getRenderContext().getCanvas().clientLeft +
                _navigation.ctx.getRenderContext().getCanvas().clientWidth / 2,
              _navigation.ctx.getRenderContext().getCanvas().clientTop +
                _navigation.ctx.getRenderContext().getCanvas().clientHeight / 2
            ];
            pos2 = _navigation.ctx.getPixelFromLonLat(_pressedGeo[0], _pressedGeo[1]);
...
export default GoogleMouseNavigationHandler;
