...
var FlatNavigation = function (ctx, options) {
  ...
  this.maxDistance =
    ...
    (5.0 * this.ctx.getCoordinateSystem().getGeoide().getRadius()) /
      this.ctx.getCoordinateSystem().getGeoide().getHeightScale();
...
function _addZoomOutThenZoomIn(zoomToAnimation, startValue, endValue, maxAltitude) {
  ...
  var midValue = [startValue[0] * 0.5 + endValue[0] * 0.5, startValue[1] * 0.5 + endValue[1] * 0.5, maxAltitude];
...
FlatNavigation.prototype.zoomTo = function (geoPos, options) {
  ...
  var destDistance =
    options && options.distance
      ? options.distance
      : this.distance / this.ctx.getCoordinateSystem().getGeoide().getHeightScale();
