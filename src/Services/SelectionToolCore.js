  ...
  this.renderContext.canvas.addEventListener("mousemove", function (event) {
    ...
    if (state === "resize") {
      ...
      self.radius = Math.sqrt(
        Math.pow(event.layerX - self.pickPoint[0], 2) + Math.pow(event.layerY - self.pickPoint[1], 2)
      );
...
SelectionToolCore.prototype.computeGeoRadius = function (pt) {
  ...
  var dotProduct = vec3.dot(
    ...
    vec3.normalize(this.coordinateSystem.get3DFromWorld(this.geoPickPoint))
  );
...
SelectionToolCore.prototype.computeSelection = function () {
  ...
  var points = [
    [
      ...
      (rc.canvas.height - this.pickPoint[1] - this.radius) * heightScale - 1.0,
      1,
      1
    ],
    [
...
export default SelectionToolCore;
