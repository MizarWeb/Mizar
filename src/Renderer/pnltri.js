...
PNLTRI.Math = {
  ...
  ptsCrossProd: function (inPtVertex, inPtFrom, inPtTo) {
    ...
    return (
      (inPtFrom.x - inPtVertex.x) * (inPtTo.y - inPtVertex.y) - (inPtFrom.y - inPtVertex.y) * (inPtTo.x - inPtVertex.x)
    );
          ...
          if (this.fpEqual(ptMain.y, ptOther.y)) {
            ...
            if (!this.fpEqual(qsNode.seg.vFrom.y, qsNode.seg.vTo.y)) {
              ...
              if (ptMain == qsNode.seg.vFrom) {
                ...
                qsNode = (isInSegmentShorter ? inSegment.sprev.upward : qsNode.seg.snext.upward)
                  ? qsNode.right
                ...
                isInSegmentShorter = inSegment.upward
                  ...
                  : ptOther.x >= qsNode.seg.vFrom.x;
                qsNode = (isInSegmentShorter ? inSegment.snext.upward : qsNode.seg.sprev.upward)
                  ? qsNode.left
                  : qsNode.right; // below : above
              }
            }
