...
import RendererTileData from "./RendererTileData";
...
RasterOverlayRenderer.prototype.updateOverlay = function (overlay) {
  ...
  if (
    overlay.containsDimension("time") &&
    overlay.time &&
    !overlay.time.isInTimeDefinition(overlay.getDimensions().time.value)
  ) {
    // pass
  } else {
  ...
  for (var n = start; n < end; n++) {
    ...
    var indexBuffer =
      renderable.tile.state === Tile.State.LOADED
        ...
        : this.tileManager.tileIndexBuffer.getSubSolid(renderable.tile.parentIndex);
...
export default RasterOverlayRenderer;
