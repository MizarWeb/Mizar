...
import DefaultNameResolver from "./DefaultNameResolver";
// import "jquery.ui";
import "./CDSNameResolver";
import "./DictionaryNameResolver";
import "./IMCCENameResolver";
/**
 * plugin:not_found.<br/>
...
function zoomTo(lon, lat, distanceCamera, crs, callback, args) {
  if (args !== null && typeof args !== "undefined") {
    ...
    while (idx < args.features.length) {
      args.features[idx].geometry.coordinates = mizarAPI
        .getCrs()
        .convert(args.features[idx].geometry.coordinates, crs, mizarAPI.getCrs().getGeoideName());
  ...
  if (mizarAPI.getActivatedContext().getMode() === Constants.CONTEXT.Sky) {
    ...
    var distance =
      distanceCamera == null ? mizarAPI.getActivatedContext().getNavigation().getDistance() : distanceCamera;
