  ...
  WCS.Mapper = (function () {
    ...
    Mapper.prototype.verifyHeader = function (header) {
      ...
      for (axis = _i = 1; 1 <= naxis ? _i <= naxis : _i >= naxis; axis = 1 <= naxis ? ++_i : --_i) {
        for (
          j = _j = 0, _ref = requiredCards.length - 1;
      ...
      date = new Date();
      this.wcsobj.dateObs =
        header["DATE_OBS"] || date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate();
      this.wcsobj.dateObs =
        header["DATE_OBS"] || "" + date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate();
    ...
    Mapper.prototype.setProjection = function (header) {
      ...
      if (((_ref = this.projection), __indexOf.call(zenithal, _ref) >= 0)) {
        ...
        switch (this.projection) {
          ...
          case "ARC":
            ...
            break;
          case "AZP":
            (_ref1 = ["PV" + this.latitudeAxis + "_1", "PV" + this.latitudeAxis + "_2"]),
              (key1 = _ref1[0]),
              (key2 = _ref1[1]);
          ...
          case "NCP":
            ...
            break;
          case "SIN":
            (_ref2 = ["PV" + this.latitudeAxis + "_1", "PV" + this.latitudeAxis + "_2"]),
              (key1 = _ref2[0]),
              (key2 = _ref2[1]);
          ...
          case "SZP":
            (_ref3 = [
              "PV" + this.latitudeAxis + "_1",
              "PV" + this.latitudeAxis + "_2",
              "PV" + this.latitudeAxis + "_3"
            ]),
              (key1 = _ref3[0]),
              (key2 = _ref3[1]),
              (key3 = _ref3[2]);
            ...
            this.fromSpherical = function (phi, theta) {
              ...
              x =
                ((180 / Math.PI) *
                  (_this.wcsobj.zp * WCS.Math.cosd(theta) * WCS.Math.sind(phi) -
                    _this.wcsobj.xp * (1 - WCS.Math.sind(theta)))) /
                divisor;
              y =
                ((-180 / Math.PI) *
                  (_this.wcsobj.zp * WCS.Math.cosd(theta) * WCS.Math.cosd(phi) +
                    _this.wcsobj.yp * (1 - WCS.Math.sind(theta)))) /
                divisor;
              return [x, y];
            };
            break;
          ...
          case "TAN-SIP":
            ...
            this.toIntermediate = function (points) {
              ...
              for (
                i = _i = 0, _ref4 = _this.wcsobj.naxis - 1;
                0 <= _ref4 ? _i <= _ref4 : _i >= _ref4;
                i = 0 <= _ref4 ? ++_i : --_i
              ) {
                proj[i] = 0;
                points[i] -= _this.wcsobj.crpix[i];
                for (
                  j = _j = 0, _ref5 = _this.wcsobj - 1;
            ...
            this.fromIntermediate = function (proj) {
              ...
              for (
                ...
                tmp[i] = 0;
                for (
                  j = _j = 0, _ref5 = _this.wcsobj.naxis - 1;
                  0 <= _ref5 ? _j <= _ref5 : _j >= _ref5;
                  j = 0 <= _ref5 ? ++_j : --_j
                ) {
      ...
      if (((_ref4 = this.projection), __indexOf.call(cylindrical, _ref4) >= 0)) {
        ...
        switch (this.projection) {
          case "CYP":
            (_ref5 = ["PV" + this.latitudeAxis + "_1,", "PV" + this.latitudeAxis + "_2"]),
              (key1 = _ref5[0]),
              (key2 = _ref5[1]);
            ...
            this.fromSpherical = function (phi, theta) {
              ...
              y =
                (180 / Math.PI) *
                ((_this.wcsobj.mu + _this.wcsobj.lambda) / (_this.wcsobj.mu + WCS.Math.cosd(theta))) *
                WCS.Math.sind(theta);
              return [x, y];
            };
            break;
          ...
          case "MOL":
            this.toSpherical = function (x, y) {
              var phi, theta;
              theta = WCS.Math.asind(
                WCS.Math.asind((Math.PI * y) / (180 * Math.sqrt(2))) / 90 +
                  (y / 180) * Math.sqrt(2 - Math.pow((Math.PI * y) / 180, 2))
              );
    ...
    Mapper.prototype.getSipCoefficients = function (header) {
      ...
      for (i = _m = 0, _ref4 = this.sip.apOrder; 0 <= _ref4 ? _m <= _ref4 : _m >= _ref4; i = 0 <= _ref4 ? ++_m : --_m) {
        this.sip.apCoeffs[i] = [];
        for (
          j = _n = 0, _ref5 = this.sip.apOrder;
          0 <= _ref5 ? _n <= _ref5 : _n >= _ref5;
          j = 0 <= _ref5 ? ++_n : --_n
        ) {
          key = "AP_" + i + "_" + j;
      ...
      for (i = _o = 0, _ref6 = this.sip.bpOrder; 0 <= _ref6 ? _o <= _ref6 : _o >= _ref6; i = 0 <= _ref6 ? ++_o : --_o) {
        this.sip.bpCoeffs[i] = [];
        for (
          j = _p = 0, _ref7 = this.sip.bpOrder;
          0 <= _ref7 ? _p <= _ref7 : _p >= _ref7;
          j = 0 <= _ref7 ? ++_p : --_p
        ) {
          key = "BP_" + i + "_" + j;
    ...
    Mapper.prototype.computeCelestialParameters = function (phi0, theta0) {
      ...
      deltaP1 = WCS.Math.atan2d(
        WCS.Math.sind(this.wcsobj.theta0),
        WCS.Math.cosd(this.wcsobj.theta0 * WCS.Math.cosd(phiP - this.wcsobj.phi0))
      );
      deltaP2 = WCS.Math.acosd(
        WCS.Math.sind(delta0) /
          Math.sqrt(
            1 - Math.pow(WCS.Math.cosd(this.wcsobj.theta0), 2) * Math.pow(WCS.Math.sind(phiP - this.wcsobj.phi0), 2)
          )
      );
      sol1 = sol2 = false;
      ...
      return (this.wcsobj.alphaP =
        Math.abs(delta0) === 90
          ? alpha0
          : alpha0 -
            WCS.Math.asind(
              (WCS.Math.sind(phiP - this.wcsobj.phi0) * WCS.Math.cosd(this.wcsobj.theta0)) / WCS.Math.cosd(delta0)
            ));
    };
    ...
    Mapper.prototype.toIntermediate = function (points) {
      ...
      for (i = _i = 0, _ref = this.wcsobj.naxis - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
        ...
        for (
          j = _j = 0, _ref1 = this.wcsobj.naxis - 1;
          0 <= _ref1 ? _j <= _ref1 : _j >= _ref1;
          j = 0 <= _ref1 ? ++_j : --_j
        ) {
