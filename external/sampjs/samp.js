    ...
    for (i = 0; i < children.length; i++) {
      ...
      if (child.nodeType === 1) {
        ...
        if (childTagName && child.tagName !== childTagName) {
          throw new Error(
            "Child <" + children[i].tagName + ">" + " of <" + el.tagName + ">" + " is not a <" + childTagName + ">"
          );
        }
        childEls.push(child);
      }
  ...
  CallableClient.prototype.receiveCall = function (senderId, msgId, message) {
    ...
    if (mtype in this.callHandler) {
      try {
        ...
        response = { "samp.status": "samp.ok", "samp.result": result };
        handled = true;
      } catch (e) {
        response = { "samp.status": "samp.error", "samp.error": { "samp.errortxt": e.toString() } };
      }
    } else {
      response = { "samp.status": "samp.warning", "samp.result": {}, "samp.error": { "samp.errortxt": "no action" } };
    }
